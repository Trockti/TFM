...
export class ApiService {
  ...
  url = "http://10.117.129.37:3232/api"
  ...
  getDefinition(word: string, phrase: string): Promise<any>{
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        //       if(definition_list.length == 0){
        //         definition = await this.getDefinitionAnglicism(word)
        ...
        definition = await this.getDefinitionAnglicism(word, phrase)
        resolve({definition, source: 'anglicism'})
  ...
  getDefinitionAnglicism(word: string, phrase: string):Promise<string>{
    return new Promise<string>((resolve, reject) => {
            try {
        this.http.get(this.url+'/anglicism', {
          params: {
            word: word,
            phrase: phrase
          }
