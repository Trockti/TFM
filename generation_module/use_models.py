...
def buscar_en_dataset(palabra, ruta_json="../data/dictionaries/normalized_definitions.json"):
    """
    ...
    try:
        with open("../data/dictionaries/definitions_v2.json", 'r', encoding='utf-8') as f:
            datos = json.load(f)
    except json.JSONDecodeError:
        print("Error al leer el JSON. Asegúrate de que el formato sea correcto.")
        return None
...
def main() -> None:
    ...
    input_file = "../data/dictionaries/transformed_terms.json" 
    try:
...
"""
    ...
    for idx, prompt_template in enumerate(prompt_templates, start=1):
        folder_name = f"experiments/results_v{idx}"  
